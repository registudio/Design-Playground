...
import { BrandFromUrl } from "@/components/BrandFromUrl";
...
export default function Playground() {
  ...
  const [lensSingle, setLensSummary] = useState<ContrastSummary | null>(null);
  // Light and dark next to each other: the contrast lens runs in both, counted together.
  const [split, setSplit] = useState(false);
  const [lensLight, setLensLight] = useState<ContrastSummary | null>(null);
  const [lensDark, setLensDark] = useState<ContrastSummary | null>(null);
  const lensSummary = !split ? lensSingle : lensLight && lensDark
    ? { checked: lensLight.checked + lensDark.checked, failing: lensLight.failing + lensDark.failing, unknown: lensLight.unknown + lensDark.unknown }
    : null;
